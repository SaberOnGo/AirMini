...
void USB_Interrupts_Config(void)
{
 ...
 NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);
  ...
  #else
  STM32_NVICInit(USB_LP_CAN1_RX0_IRQn, 2, 2, 0);	 // 第2组优先级, 2位抢占优先级, 2位响应优先级
  STM32_NVICInit(USB_HP_CAN1_TX_IRQn,  2, 1, 0);	 // 第1组优先级, 1位抢占优先级, 3位响应优先级
  #endif
...
void USB_Cable_Config (FunctionalState NewState)
{
  if (NewState == DISABLE)
  {
    GPIOA->BRR = GPIO_Pin_12; //GPIO_ResetBits(GPIOA, GPIO_Pin_12);
  }
  else
  {
    GPIOA->BSRR = GPIO_Pin_12; //GPIO_SetBits(GPIOA, GPIO_Pin_12);
  }
}
