...
static int32_t gizProtocolGetDeviceInfo(protocolHead_t * head)
{
    ...
    #else
    memcpy((uint8_t *)deviceInfo.productKey, PRODUCT_KEY_STRING(),   strlen(PRODUCT_KEY_STRING()));
    memcpy((uint8_t *)deviceInfo.productSecret, PRODUCT_SECRET_STRING(), strlen(PRODUCT_SECRET_STRING()));
    #endif
